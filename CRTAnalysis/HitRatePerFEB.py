...
avg = reduce(lambda x, y: x + y, mean_top_long) / len(mean_top_long)
print "Average of Means -------------> ", avg
...
avg = reduce(lambda x, y: x + y, mean_top_short) / len(mean_top_short)
print "Average of Means -------------> ", avg
print "-----------------------------------------------------"
...
for l in ft_longModules:
    tempHist    = inFile.Get("crt/hEvtRateFT_"+l)
    tempHist.SetLineColor(kRed)
    tempHist.SetLineWidth(2)
    hFTHistos_ft_long.append(tempHist)

for s in ft_shortModules:
    tempHist    = inFile.Get("crt/hEvtRateFT_"+s)
    tempHist.SetLineColor(kBlue)
    tempHist.SetLineWidth(2)
    hFTHistos_ft_short.append(tempHist)

cFT = TCanvas("cFT","the fit canvas",600,600);
cFT.cd()
mean_ft_long = []
mean_ft_short = []
print "FT_Long Modules Means:"
for h in hFTHistos_ft_long:
    h.GetXaxis().SetRangeUser(0, 20)
    h.GetYaxis().SetRangeUser(0, 1000)
    h.GetYaxis().SetTitleOffset(1.5)
    h.Draw("sames")
    mean = h.GetMean()
    print h.GetTitle(), "---> ", mean
    mean_ft_long.append(mean)
    h.SetTitle("Rate for FT Modules; N hits in 4 ms; N events")

print "Average of Means -------------> ", reduce(lambda x, y: x + y, mean_ft_long) / len(mean_ft_long)

print
print "FT_Short Modules Means:"
for h in hFTHistos_ft_short:
    h.Draw("sames")
    mean = h.GetMean()
    print h.GetTitle(), "---> ", mean
    mean_ft_short.append(mean)


legendFT = TLegend(.54,.52,.84,.70);
legendFT.AddEntry(hFTHistos_ft_short[1]   ,"FT_Short Modules ");
legendFT.AddEntry(hFTHistos_ft_long[1]   ,"FT_Long Modules ");
legendFT.Draw("same")
print "Average of Means -------------> ", reduce(lambda x, y: x + y, mean_ft_short) / len(mean_ft_short)
print "-----------------------------------------------------"
...
hHitTimeDiffFT.Write()
hTimeVsXVsZ_Bottom.Write()
hTimeVsXVsZ_FTy.Write()
outFile.Write()
outFile.Close()
'''
